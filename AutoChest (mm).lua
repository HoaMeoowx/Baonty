-- Tạo GUI để bật/tắt tính năng farm chest
...
ScreenGui.Name = "ToggleFarmChestGUI"
...
EnableButton.Text = "Bật Farm Chest"
...
DisableButton.Size = UDim2.new(0, 180, 0, 30)
DisableButton.Text = "Tắt Farm Chest"
DisableButton.TextColor3 = Color3.fromRGB(255, 255, 255)

-- Biến trạng thái bật/tắt farm chest
getgenv().FarmChestRunning = false

-- Logic farm chest
local function startFarmChest()
    if getgenv().FarmChestRunning then return end
    getgenv().FarmChestRunning = true
    spawn(function()
        while getgenv().FarmChestRunning do
            -- Logic farm chest
            for _, chest in pairs(game.Workspace:GetDescendants()) do
                if chest:IsA("Model") and chest.Name == "Chest" and chest:FindFirstChild("TouchInterest") then
                    game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = chest.CFrame
                    wait(1) -- Thời gian chờ giữa các lần farm
                end
            end
            wait(0.5) -- Chờ trước khi quét lại các chest
        end
    end)
end

local function stopFarmChest()
    getgenv().FarmChestRunning = false
end

-- Kết nối các nút với hành động farm chest
