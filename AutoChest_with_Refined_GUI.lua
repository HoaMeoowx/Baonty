...
EnableButton.Text = "Bật Tính Năng"
...
DisableButton.Text = "Tắt Tính Năng"
DisableButton.TextColor3 = Color3.fromRGB(255, 255, 255)

-- Biến trạng thái bật/tắt
getgenv().ScriptRunning = false

-- Chức năng Auto Rejoin
local function autoRejoin()
    while getgenv().ScriptRunning do
        wait(0.1)
        game:GetService("CoreGui").RobloxPromptGui.promptOverlay.ChildAdded:Connect(function(v661)
...
local function randomBehavior()
    while getgenv().ScriptRunning do
        wait(0.1)
        local v138_ = math.random(1, 5)
        wait(0.3)
    end
end

-- Bắt đầu tất cả tính năng
local function startScripts()
    if getgenv().ScriptRunning then return end
    getgenv().ScriptRunning = true
    spawn(autoRejoin)
    spawn(randomBehavior)
end

-- Dừng tất cả tính năng
